...
GLiREL integration (Stage 2b):
  ...
  After GLiNER NER, extract_with_glirel() runs on the same articles.

  raw_glirel_triples passed to GraphBuilder so every GLiREL edge becomes

  a real graph edge + Neo4j row.

Wikipedia ingestion (--wiki):

  Calls wiki_loader.fetch_all() + ingest_to_db() instead of NewsDownloader.

  --input is automatically set to data/raw/wiki when --wiki is used.
"""
...
def _run_wiki_download(args) -> str:

    """

    Fetch Wikipedia articles via wiki_loader and return the output directory.

    Imported lazily so the heavy wikipedia + requests deps are only loaded

    when --wiki is actually passed.
...
def run_pipeline(args):
    ...
    if args.wiki:
        ...
        wiki_dir = _run_wiki_download(args)

        # Override input dir so load_from_directory picks up wiki files

        if not args.download:          # only override when GNews not also requested

            args.input = wiki_dir

    # --download: GNews fetch (original behaviour, unchanged)
    ...
    # Load from directory (covers both wiki JSONL + GNews JSON outputs)
    ...
    if args.from_stage <= 2 and not args.skip_glirel and extractor is not None:
        ...
        try:
            ...
            # Persist so --from-stage 9 can reload without re-running GLiREL
            import pickle
    ...
    if args.from_stage <= 4:
        ...
        # BUG #1 FIX: metadata aligned to embedder output, not unprocessed list
    ...
    if args.from_stage <= 7:
        ...
        # Reload GLiREL triples pickled during stage 2b
        import pickle
    ...
    # BUG #7 FIX: define before try block
...
if __name__ == "__main__":
    ...
    # ── Shared date range (used by both --wiki and --download) ─────────
    ...
    # Auto-set input dir when only --wiki is used (no --download)
